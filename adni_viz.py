...
PAGE_CSS = f"""
  * {{ box-sizing: border-box; }}
  ...
  /* header-info-card grows to fill the stretched column; its own LAST
     section (the population filter row) absorbs the extra height
     rather than leaving a blank gap after it. */
  .header-left-col .header-info-card {{ flex: 1; display: flex; flex-direction: column; }}
  .header-info-filter-row {{ flex: 1; }}
  ...
  /* Unified header info card: description/disclaimer, participant
     stats, status legend, and the population filter used to each be
     their own separately-shadowed, differently-rounded floating box
     (a rectangular note, two pill-ish cards, and one true 999px pill),
     stacked with visible gaps -- inconsistent radii and a repeated
     white/shadow treatment four times over. Now ONE card, ONE radius,
     ONE shadow, with thin internal dividers between sections instead
     of a new box each time. */
  .header-info-card {{
    background: white; border-radius: {CARD_RADIUS}; box-shadow: {CARD_SHADOW};
    margin-bottom: 14px; overflow: hidden;
  }}
  .header-info-section {{ padding: 13px 20px; border-top: 1px solid {SURFACE_BORDER}; }}
  .header-info-section:first-child {{ border-top: none; }}

  .header-info-disclaimer {{
    font-size: 12.5px; color: #33475b; line-height: 1.55;
    border-left: 3px solid {ARIBIO_BLUE}; padding-left: 17px;
  }}
  .header-info-stats {{ font-size: 14px; font-weight: 600; color: #1a1a1a; }}
  .header-info-stats b {{ color: {ARIBIO_BLUE}; }}

  /* Status legend -- symbols only, full definitions on hover (title
     attr) and in the Analysis Details disclosure. */
  .header-info-legend {{ display: flex; flex-wrap: wrap; gap: 16px; align-items: center; font-size: 12.5px; color: #444; }}
  ...
  /* Population filter -- now just another full-width section of the
     same card (no separate pill-shaped container of its own, so it no
     longer sits narrower than its siblings). */
  .header-info-filter-row {{ display: flex; align-items: center; flex-wrap: wrap; column-gap: 12px; row-gap: 6px; }}
  .population-filter-icon {{ color: {ARIBIO_BLUE}; flex-shrink: 0; }}
  .population-filter-label {{ font-size: 13px; font-weight: 700; color: {ARIBIO_BLUE}; white-space: nowrap; }}
  ...
  /* Population filter -- explicitly labeled "Filter by population"
     (not just "Population") so it reads as an action that changes
     what the page shows, not a passive info label. Now a section of
     .header-info-card (see above) rather than its own separately
     boxed/shadowed pill. */
  ...
  /* The Overall/POLARIS helper sentences are very different lengths --
     the POLARIS one wraps onto an extra line at typical widths. Both
     are stacked in the SAME grid cell (grid-area: 1/1) at all times,
     so the wrapper's height is always the taller of the two regardless
     of which one is toggled visible. */
  .population-helper-stack {{ display: grid; flex: 1 1 240px; }}
  .population-helper-stack > .population-helper {{ grid-area: 1 / 1; }}
  .population-helper {{ font-size: 12.5px; color: #666; line-height: 1.4; }}

  /* Same idea for the POLARIS Cohort Construction toggle's hint text --
     the "enabled" sentence is much longer than the "disabled" one. */
  .polaris-hint-stack {{ display: grid; }}
  .polaris-hint-stack > .polaris-hint {{ grid-area: 1 / 1; margin: 4px 0 0; }}
  ...
  /* Now a section of .header-info-card (right under the filter row) --
     padding-left trimmed by the border width so its text still lines
     up with the other sections' 20px inset. ALWAYS rendered (never
     display:none) at a fixed height, whether Overall ADNI or POLARIS
     is active -- switching population only toggles the disabled/
     enabled look below, so the header card's own height (and
     everything below it on the page) never shifts just from changing
     the population filter. Only a genuine user click on the toggle
     (enabled state only) is allowed to change layout height. */
  .polaris-panel {{ border-left: 4px solid {ARIBIO_ACCENT}; padding-left: 16px; }}
  .polaris-panel--disabled .collapsible-toggle {{ cursor: default; pointer-events: none; opacity: 0.55; }}
  .polaris-panel--disabled .chev {{ visibility: hidden; }}
  .polaris-context-box {{
...
def render_header_section():
    ...
    <div class="header-info-card">
      ...
      <div class="header-info-section header-info-stats">
        <b>3,030</b> participants &middot; CN <b>1,215</b> &middot; MCI <b>1,338</b> &middot; Dementia <b>477</b> &middot; <b>7</b> endpoints &middot; up to <b>48</b> months
      </div>
      ...
      <div class="header-info-section header-info-filter-row">
        <svg class="population-filter-icon" width="13" height="13" viewBox="0 0 16 16" aria-hidden="true" focusable="false">
          <path d="M1 2.5h14L9.5 9v4.5l-3 1.5V9L1 2.5z" fill="currentColor"></path>
        </svg>
        <span class="population-filter-label" title="Changes which validated cohort every chart on this page shows.">Filter by population:</span>
        ...
        <span class="population-helper-stack">
          <span class="population-helper" id="populationHelperOverall">Broad validated ADNI natural-history cohort.</span>
          <span class="population-helper is-hidden" id="populationHelperPolaris">ADNI participants meeting validated MMSE and amyloid-PET eligibility criteria relevant to the POLARIS clinical-development population.</span>
        </span>
      </div>
      <div class="header-info-section polaris-panel polaris-panel--disabled" id="polarisPanel">
        <div class="collapsible-toggle" id="polarisToggleHeader" onclick="toggleCollapsible(this)">
          <div>
            <h2 style="margin:0;font-size:14px;">POLARIS AD&ndash;Aligned Cohort Construction</h2>
            <div class="polaris-hint-stack">
              <p class="panel-sub polaris-hint" id="polarisHintDisabled">Select POLARIS AD&ndash;Aligned above to view how the 620-participant cohort was built.</p>
              <p class="panel-sub polaris-hint is-hidden" id="polarisHintEnabled">Eligibility filtering only &mdash; not propensity-score matching, not an external control arm. Click to see how the 620-participant cohort was built.</p>
            </div>
          </div>
...
function setPopulation(pop) {
  ...
  // visibility, not display -- both helper sentences stay in the
  // layout (see .population-helper-stack) so the row's height never
  // changes when switching population.
  document.getElementById("populationHelperOverall").classList.toggle("is-hidden", pop !== "overall");
  document.getElementById("populationHelperPolaris").classList.toggle("is-hidden", pop !== "polaris");
  ...
  // The POLARIS Cohort Construction toggle stays in the document at a
  // fixed height at all times (never display:none) -- only its
  // enabled/disabled look changes here, so switching population never
  // shifts the header card's height or anything below it on the page.
  // Its own collapsible body only expands on a real user click.
  const polarisPanelEl = document.getElementById("polarisPanel");
  const polarisToggleHeader = document.getElementById("polarisToggleHeader");
  polarisPanelEl.classList.toggle("polaris-panel--disabled", pop !== "polaris");
  // Both hint sentences stay in the layout (see .polaris-hint-stack) --
  // toggled by visibility, not textContent, since the "enabled" one is
  // much longer and would otherwise wrap onto an extra line.
  document.getElementById("polarisHintDisabled").classList.toggle("is-hidden", pop === "polaris");
  document.getElementById("polarisHintEnabled").classList.toggle("is-hidden", pop !== "polaris");
  // Leaving POLARIS always resets the toggle back to collapsed, so
  // re-selecting POLARIS later never shows it already expanded.
  if (pop !== "polaris") {
    polarisToggleHeader.classList.remove("open");
    polarisToggleHeader.nextElementSibling.classList.remove("open");
  }
