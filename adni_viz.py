...
PAGE_CSS = f"""
  * {{ box-sizing: border-box; }}
  ...
  /* header-info-card grows to fill the stretched column; the bottom
     (cohort) card grows within it, and POLARIS panel -- its own last
     child -- absorbs the extra height, rather than leaving a blank gap
     after it. */
  ...
  /* Population filter -- the one control in this card. No background
     wash of its own -- the accent color already lives in the icon,
     label, and the toggle button's own solid-blue active state, so a
     tinted strip behind it would be redundant, not additive. Generous
     padding (the actual "blank space" doing the separating work) plus
     a hairline divider is what sets it apart from its neighbors. */
  .header-info-filter-row {{
    display: flex; align-items: center; flex-wrap: wrap; column-gap: 12px; row-gap: 6px;
    border-top: 1px solid {SURFACE_BORDER}; padding: 16px 20px;
  }}
  .population-filter-icon {{ color: {ARIBIO_BLUE}; flex-shrink: 0; }}
  .population-filter-label {{ font-size: 15px; font-weight: 700; color: {ARIBIO_BLUE}; white-space: nowrap; }}
  ...
  .population-helper {{ font-size: 13px; color: #445; line-height: 1.4; }}
  ...
  /* Nested inside .header-info-cohort-card, directly under the filter
     row, no gap and no background of its own -- the shared card plus a
     hairline divider is what makes clear it belongs to the filter
     above it, rather than being an unrelated section. Its title uses
     ...
     own distinct sub-topic without a colored fill. flex: 1 lets it
     absorb any extra stretch height from matching Disease Continuum's
     height. ALWAYS rendered (never display:none) at a fixed height,
     whether Overall ADNI or POLARIS is active -- switching population
     only toggles the disabled/enabled look below, so the header card's
     own height (and everything below it on the page) never shifts just
     from changing the population filter. Only a genuine user click on
     the toggle (enabled state only) is allowed to change layout height. */
  .polaris-panel {{ border-top: 1px solid {SURFACE_BORDER}; padding: 16px 20px; flex: 1; }}
  .polaris-panel h2 {{ color: {ARIBIO_ACCENT}; }}
      ...
      <div class="header-info-cohort-card">
        ...
        <div class="header-info-section polaris-panel polaris-panel--disabled" id="polarisPanel">
          <div class="collapsible-toggle" id="polarisToggleHeader" onclick="toggleCollapsible(this)">
            <div>
              <h2 style="margin:0;font-size:15px;">POLARIS AD&ndash;Aligned Cohort Construction</h2>
